...
This is separate from `~/.clawdia/`, which stores config, credentials, and
sessions.
...
- Default: `~/clawd`
- If `CLAWDIA_PROFILE` is set and not `"default"`, the default becomes
  `~/clawd-<profile>`.
- Override in `~/.nelsonmuntz-c/clawdia.json`:
...
If any bootstrap file is missing, Clawdia injects a "missing file" marker into
...
- `~/.clawdia/credentials/` (OAuth tokens, API keys)
- `~/.clawdia/agents/<agentId>/sessions/` (session transcripts + metadata)
- `~/.clawdia/skills/` (managed skills)
...
- Anything under `~/.clawdia/`.
- Raw dumps of chats or sensitive attachments.

If you must store sensitive references, use placeholders and keep the real
secret elsewhere (password manager, environment variables, or `~/.clawdia/`).
...
2. Set `agents.defaults.workspace` to that path in `~/.nelsonmuntz-c/clawdia.json`.
3. Run `clawdia setup --workspace <path>` to seed any missing files.
4. If you need sessions, copy `~/.clawdia/agents/<agentId>/sessions/` from the
