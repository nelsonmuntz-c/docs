...
- `/usage cost` in chats: local cost summary aggregated from Clawdia session logs.
- CLI: `clawdia status --usage` prints a full per-provider breakdown.
- CLI: `clawdia channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
