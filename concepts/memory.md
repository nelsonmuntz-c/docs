---
summary: "How Clawdia memory works (workspace files + automatic memory flush)"
read_when:
...
Clawdia memory is **plain Markdown in the agent workspace**. The files are the
...
When a session is **close to auto-compaction**, Clawdia triggers a **silent,
...
Clawdia can build a small vector index over `MEMORY.md` and `memory/*.md` so
...
Remote embeddings **require** an API key for the embedding provider. Clawdia
...
Clawdia can cache **chunk embeddings** in SQLite so reindexing and frequent updates (especially session transcripts) don't re-embed unchanged text.
...
When the sqlite-vec extension is available, Clawdia stores embeddings in a
...
- If the sqlite-vec extension is missing or fails to load, Clawdia logs the
