---
summary: "Run Clawdia in a sandboxed macOS VM (local or hosted) when you need isolation or iMessage"
read_when:
  - You want Clawdia isolated from your main macOS environment
...
Run Clawdia in a sandboxed macOS VM on your existing Apple Silicon Mac using [Lume](https://cua.ai/docs/lume).
...
1. Install Lume
2. `lume create clawdia --os macos --ipsw latest`
3. Complete Setup Assistant, enable Remote Login (SSH)
4. `lume run clawdia --no-display`
5. SSH in, install Clawdia, configure channels
6. Done
...
```bash
lume create clawdia --os macos --ipsw latest
```
...
```bash
nano ~/.nelsonmuntz-c/clawdia.json
```
...
```bash
lume stop clawdia
lume run clawdia --no-display
```

The VM runs in the background. Clawdia's daemon keeps the gateway running.

To check status:

```bash
ssh youruser@192.168.64.X "clawdia status"
```
...
This is the killer feature of running on macOS. Use [BlueBubbles](https://bluebubbles.app) to add iMessage to Clawdia.
...
```bash
lume stop clawdia
lume clone clawdia clawdia-golden
```

Reset anytime:

```bash
lume stop clawdia && lume delete clawdia
lume clone clawdia-golden clawdia
lume run clawdia --no-display
```
