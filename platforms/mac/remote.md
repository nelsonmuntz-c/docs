---
summary: "macOS app flow for controlling a remote Clawdia gateway over SSH"
read_when:
...
This flow lets the macOS app act as a full remote control for a Clawdia gateway running on another host (desktop/server). It’s the app’s **Remote over SSH** (remote run) feature. All features—health checks, Voice Wake forwarding, and Web Chat—reuse the same remote SSH configuration from *Settings → General*.
...
## Notification sounds
Pick sounds per notification from scripts with `clawdia` and `node.invoke`, e.g.:
