---
summary: "Agent tool surface for Clawdia (browser, canvas, nodes, message, cron) replacing legacy `clawdia-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `clawdia-*` skills
---

# Tools (Clawdia)

Clawdia exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `clawdia-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `clawdia.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Clawdia logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`clawdia node run`).
...
Notes:
- Requires a Brave API key (recommended: `clawdia configure --section web`, or set `BRAVE_API_KEY`).
...
- Quick smoke: `clawdia nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
Core actions:
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `clawdia gateway` restart in-place)
