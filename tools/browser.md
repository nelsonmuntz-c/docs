...
Clawdia can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
```bash
clawdia browser --browser-profile clawd status
...
- `chrome`: extension relay to your **system browser** (requires the Clawdia
...
Browser settings live in `~/.nelsonmuntz-c/clawdia.json`.
...
- If you override the Gateway port (`gateway.port` or `CLAWDIA_GATEWAY_PORT`),
...
Clawdia uses it automatically. Set `browser.executablePath` to override
auto-detection:
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, Clawdia will not launch a local browser.
...
Clawdia preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

### Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, Clawdia can
...
CDP endpoints over HTTPS. You can point a Clawdia browser profile at a
...
clawdia browser serve --bind <browser-host> --port 18791 --token <token>
```
...
```bash
export CLAWDIA_BROWSER_CONTROL_TOKEN="<token>"
```
...
- `browser.controlToken` / `CLAWDIA_BROWSER_CONTROL_TOKEN` is **only** for authenticating browser control HTTP requests to `browser.controlUrl`.
...
Recommended:
- Keep `clawdia browser serve` bound to loopback (`127.0.0.1`) and publish it via Tailscale.
...
Best practice here: keep `clawdia browser serve` on HTTP and terminate TLS in front.
...
Clawdia supports multiple named profiles (routing configs). Profiles can be:
...
Clawdia can also drive **your existing Chrome tabs** (no separate “clawd” Chrome instance) via a local CDP relay + a Chrome extension.

Full guide: [Chrome extension](/tools/chrome-extension)

Flow:
- You run a **browser control server** (Gateway on the same machine, or `clawdia browser serve`).
- A local **relay server** listens at a loopback `cdpUrl` (default: `http://127.0.0.1:18792`).
- You click the **Clawdia Browser Relay** extension icon on a tab to attach (it does not auto-attach).
...
- “Load unpacked” → select the directory printed by `clawdia browser extension path`
- Pin the extension, then click it on the tab you want to control (badge shows `ON`).

2) Use it:
- CLI: `clawdia browser --browser-profile chrome tabs`
- Agent tool: `browser` with `profile="chrome"`

Optional: if you want a different name or relay port, create your own profile:

```bash
clawdia browser create-profile \
  --name my-chrome \
  --driver extension \
...
When launching locally, Clawdia picks the first available:
1. Chrome
2. Brave
3. Edge
4. Chromium
...
Basics:
- `clawdia browser status`
- `clawdia browser start`
- `clawdia browser stop`
- `clawdia browser tabs`
- `clawdia browser tab`
- `clawdia browser tab new`
- `clawdia browser tab select 2`
- `clawdia browser tab close 2`
- `clawdia browser open https://example.com`
- `clawdia browser focus abcd1234`
- `clawdia browser close abcd1234`

Inspection:
- `clawdia browser screenshot`
- `clawdia browser screenshot --full-page`
- `clawdia browser screenshot --ref 12`
- `clawdia browser screenshot --ref e12`
- `clawdia browser snapshot`
- `clawdia browser snapshot --format aria --limit 200`
- `clawdia browser snapshot --interactive --compact --depth 6`
- `clawdia browser snapshot --efficient`
- `clawdia browser snapshot --labels`
- `clawdia browser snapshot --selector "#main" --interactive`
- `clawdia browser snapshot --frame "iframe#main" --interactive`
- `clawdia browser console --level error`
- `clawdia browser errors --clear`
- `clawdia browser requests --filter api --clear`
- `clawdia browser pdf`
- `clawdia browser responsebody "**/api" --max-chars 5000`

Actions:
- `clawdia browser navigate https://example.com`
- `clawdia browser resize 1280 720`
- `clawdia browser click 12 --double`
- `clawdia browser click e12 --double`
- `clawdia browser type 23 "hello" --submit`
- `clawdia browser press Enter`
- `clawdia browser hover 44`
- `clawdia browser scrollintoview e12`
- `clawdia browser drag 10 11`
- `clawdia browser select 9 OptionA OptionB`
- `clawdia browser download e12 /tmp/report.pdf`
- `clawdia browser waitfordownload /tmp/report.pdf`
- `clawdia browser upload /tmp/file.pdf`
- `clawdia browser fill --fields '[{"ref":"1","type":"text","value":"Ada"}]'`
- `clawdia browser dialog --accept`
- `clawdia browser wait --text "Done"`
- `clawdia browser wait "#main" --url "**/dash" --load networkidle --fn "window.ready===true"`
- `clawdia browser evaluate --fn '(el) => el.textContent' --ref 7`
- `clawdia browser highlight e12`
- `clawdia browser trace start`
- `clawdia browser trace stop`

State:
- `clawdia browser cookies`
- `clawdia browser cookies set session abc123 --url "https://example.com"`
- `clawdia browser cookies clear`
- `clawdia browser storage local get`
- `clawdia browser storage local set theme dark`
- `clawdia browser storage session clear`
- `clawdia browser set offline on`
- `clawdia browser set headers --json '{"X-Debug":"1"}'`
- `clawdia browser set credentials user pass`
- `clawdia browser set credentials --clear`
- `clawdia browser set geo 37.7749 -122.4194 --origin "https://example.com"`
- `clawdia browser set geo --clear`
- `clawdia browser set media dark`
- `clawdia browser set timezone America/New_York`
- `clawdia browser set locale en-US`
- `clawdia browser set device "iPhone 14"`
...
Clawdia supports two “snapshot” styles:

- **AI snapshot (numeric refs)**: `clawdia browser snapshot` (default; `--format ai`)
  - Output: a text snapshot that includes numeric refs.
  - Actions: `clawdia browser click 12`, `clawdia browser type 23 "hello"`.
  - Internally, the ref is resolved via Playwright’s `aria-ref`.

- **Role snapshot (role refs like `e12`)**: `clawdia browser snapshot --interactive` (or `--compact`, `--depth`, `--selector`, `--frame`)
  - Output: a role-based list/tree with `[ref=e12]` (and optional `[nth=1]`).
  - Actions: `clawdia browser click e12`, `clawdia browser highlight e12`.
...
- Wait for URL (globs supported by Playwright):
  - `clawdia browser wait --url "**/dash"`
- Wait for load state:
  - `clawdia browser wait --load networkidle`
- Wait for a JS predicate:
  - `clawdia browser wait --fn "window.ready===true"`
- Wait for a selector to become visible:
  - `clawdia browser wait "#main"`
...
1. `clawdia browser snapshot --interactive`
2. Use `click <ref>` / `type <ref>` (prefer role refs in interactive mode)
3. If it still fails: `clawdia browser highlight <ref>` to see what Playwright is targeting
...
5. For deep debugging: record a trace:
   - `clawdia browser trace start`
   - reproduce the issue
   - `clawdia browser trace stop` (prints `TRACE:<path>`)
...
```bash
clawdia browser status --json
clawdia browser snapshot --interactive --json
clawdia browser requests --filter api --json
clawdia browser cookies --json
```
