...
- `commands.config` (default `false`) enables `/config` (reads/writes `clawdia.json`).
...
- For full provider usage breakdown, use `clawdia status --usage`.
- `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from Clawdia session logs.
...
Notes:
- Overrides apply immediately to new config reads, but do **not** write to `clawdia.json`.
...
/config set messages.responsePrefix="[clawdia]"
