...
- Jobs persist under `~/.clawdia/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.clawdia/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.clawdia/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
clawdia cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
clawdia cron edit <jobId> --agent ops
clawdia cron edit <jobId> --clear-agent
```
```
...
```bash
clawdia system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`clawdia system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `CLAWDIA_SKIP_CRON`.
