...
Clawdia can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. Clawdia requires a shared password.
...
- `token` (default when `CLAWDIA_GATEWAY_TOKEN` is set)
- `password` (shared secret via `CLAWDIA_GATEWAY_PASSWORD` or config)

When `tailscale.mode = "serve"` and `gateway.auth.allowTailscale` is `true`,
valid Serve proxy requests can authenticate via Tailscale identity headers
(`tailscale-user-login`) without supplying a token/password. Clawdia only
...
Prefer `CLAWDIA_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
clawdia gateway --tailscale serve
clawdia gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want Clawdia to undo `tailscale serve`
