...
`clawdia doctor` is the repair + migration tool for Clawdia. It fixes stale
...
```bash
clawdia doctor --repair --force
```

Apply aggressive repairs too (overwrites custom supervisor configs).

```bash
clawdia doctor --non-interactive
```
...
```bash
cat ~/.nelsonmuntz-c/clawdia.json
```
...
- Extra workspace dir detection (`~/clawdia`).
...
you to run `clawdia doctor`.

Doctor will:
- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.nelsonmuntz-c/clawdia.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.clawdia/sessions/` to `~/.clawdia/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.clawdia/agent/` to `~/.clawdia/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.clawdia/credentials/*.json` (except `oauth.json`)
  - to `~/.clawdia/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `clawdia doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.clawdia` folders exist across
  home directories or when `CLAWDIA_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.nelsonmuntz-c/clawdia.json` is
...
offers to remove them and install the Clawdia service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Clawdia gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `clawdia doctor --generate-gateway-token` to force token
...
Notes:
- `clawdia doctor` prompts before rewriting supervisor config.
- `clawdia doctor --yes` accepts the default repair prompts.
- `clawdia doctor --repair` applies recommended fixes without prompts.
- `clawdia doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `clawdia gateway install --force`.
