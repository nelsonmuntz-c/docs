---
summary: "All configuration options for ~/.nelsonmuntz-c/clawdia.json with examples"
read_when:
...
Clawdia only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `clawdia doctor`, `clawdia logs`, `clawdia health`, `clawdia status`, `clawdia service`, `clawdia help`).
- Run `clawdia doctor` to see the exact issues.
- Run `clawdia doctor --fix` (or `--yes`) to apply migrations/repairs.
...
```bash
clawdia gateway call config.get --params '{}' # capture payload.hash
clawdia gateway call config.apply --params '{
...
```bash
clawdia gateway call config.get --params '{}' # capture payload.hash
clawdia gateway call config.patch --params '{
...
{ "$include": "/etc/clawdia/base.json5" } // absolute
...
```json5
// ~/.clawdia/clients/mueller/agents.json5
[
...
```json5
// ~/.clawdia/clients/mueller/broadcast.json5
{
...
Clawdia stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.clawdia/agents/<agentId>/agent/auth-profiles.json`)
...
- `~/.clawdia/agent/*` (migrated by `clawdia doctor` into `~/.clawdia/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `CLAWDIA_OAUTH_DIR`
- Agent dir (default agent root override): `CLAWDIA_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Clawdia imports `oauth.json` entries into `auth-profiles.json`.
...
- Default log file: `/tmp/nelsonmuntz-c/clawdia-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/nelsonmuntz-c/clawdia.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/nelsonmuntz-c/clawdia.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.clawdia/credentials/whatsapp/biz
          // authDir: "~/.clawdia/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `clawdia doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "clawdia", "clawd"] } }
    ]
  }
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `agentDir`: default `~/.clawdia/agents/<agentId>/agent`.
...
- `commands.config: true` enables `/config` (reads/writes `clawdia.json`).
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        allowFrom: ["1234567890", "your-username"], // optional DM allowlist ("open" requires ["*"])
...
Clawdia starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/clawdia" } }
}
```
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.clawdia/settings/tts.json",
      elevenlabs: {
...
Clawdia also ships a few built-in alias shorthands. Defaults only apply when the model
...
- `tools.web.search.apiKey` (recommended: set via `clawdia configure --section web`, or use `BRAVE_API_KEY` env var)
  ...
  tools: {
    elevated: {
      ...
      allowFrom: {
        ...
        discord: ["your-username", "1234567890123"]
      }
    }
  }
}
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.clawdia/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.clawdia/sandboxes",
        docker: {
          image: "clawdia-sandbox:bookworm-slim",
          containerPrefix: "clawdia-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "clawdia-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "clawdia-sandbox-browser:bookworm-slim",
          containerPrefix: "clawdia-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Clawdia uses
...
- Shortcut: `clawdia onboard --auth-choice opencode-zen`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `clawdia onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `clawdia onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `clawdia onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `clawdia onboard --auth-choice minimax-api`.
...
- Override the agent config root with `CLAWDIA_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.clawdia/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.clawdia/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.clawdia/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Clawdia stays in idle-only mode for backward compatibility.
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/clawdia" }
...
- Examples: `"/ui"`, `"/clawdia"`, `"/apps/clawdia"`.
...
- When a connection comes from one of these IPs, Clawdia uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `clawdia gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `CLAWDIA_GATEWAY_PORT` > `gateway.port` > default `18789`.
- Non-loopback binds (`lan`/`tailnet`/`auto`) require auth. Use `gateway.auth.token` (or `CLAWDIA_GATEWAY_TOKEN`).
...
- `gateway.auth.password` can be set here, or via `CLAWDIA_GATEWAY_PASSWORD` (recommended).
...
macOS app behavior:
- Clawdia.app watches `~/.nelsonmuntz-c/clawdia.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
Files watched:
- `~/.nelsonmuntz-c/clawdia.json` (or `CLAWDIA_CONFIG_PATH`)
...
- `CLAWDIA_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `clawdia --dev …` → uses `~/.clawdia-dev` + shifts ports from base `19001`
- `clawdia --profile <name> …` → uses `~/.clawdia-<name>` (port via config/env/flags)
...
Example:
```bash
CLAWDIA_CONFIG_PATH=~/.clawdia/a.json \
CLAWDIA_STATE_DIR=~/.clawdia-a \
clawdia gateway --port 19001
```
...
Gmail helper config (used by `clawdia webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "clawdia@gmail.com",
...
- Set `CLAWDIA_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Clawdia defaults `serve.path` to `/` so
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__clawdia/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__clawdia__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `CLAWDIA_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.clawdia/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.clawdia/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.clawdia/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
- a DNS server on the gateway host serving `clawdia.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `clawdia.internal` via that server

One-time setup helper (gateway host):

```bash
clawdia dns setup --apply
```
