...
- During heartbeat runs, Clawdia treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `clawdia sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Clawdia skips the heartbeat run entirely (no model call).
...
```bash
clawdia system event --text "Check for urgent follow-ups" --mode now
```
