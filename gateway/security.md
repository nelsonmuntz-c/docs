...
## Quick check: `clawdia security audit`

Run this regularly (especially after changing config or exposing network surfaces):

```bash
clawdia security audit
clawdia security audit --deep
clawdia security audit --fix
```
...
- Tighten local perms (`~/.clawdia` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... *spicy*. Here’s how to not get pwned.

Clawdia is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, Clawdia also attempts a best-effort live Gateway probe.
...
`clawdia security audit` warns when this setting is enabled.

## Local session logs live on disk

Clawdia stores session transcripts on disk under `~/.clawdia/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.clawdia` (see the audit section below). If you need
...
Clawdia can refresh the skills list mid-session:
...
- If you install plugins from npm (`clawdia plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.clawdia/extensions/<pluginId>/` (or `$CLAWDIA_STATE_DIR/extensions/<pluginId>/`).
  - Clawdia uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
clawdia pairing approve <channel> <code>
```
...
By default, Clawdia routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Clawdia has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.clawdia/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
4. **Re-run audit**
   - `clawdia security audit --deep` and confirm the report is clean.
...
- `~/.nelsonmuntz-c/clawdia.json`: `600` (user read/write only)
- `~/.clawdia`: `700` (user only)

`clawdia doctor` can warn and offer to tighten these permissions.

### 0.4) Network exposure (bind + port + firewall)

The Gateway multiplexes **WebSocket + HTTP** on a single port:
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `CLAWDIA_GATEWAY_PORT`
...
Doctor can generate one for you: `clawdia doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `CLAWDIA_GATEWAY_PASSWORD`).

Rotation checklist (token/password):
1. Generate/set a new secret (`gateway.auth.token` or `CLAWDIA_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Clawdia
...
- Clawdia will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
- and authenticate with the same token (`CLAWDIA_BROWSER_CONTROL_TOKEN` env preferred)
...
Assume anything under `~/.clawdia/` (or `$CLAWDIA_STATE_DIR/`) may contain secrets or private data:

- `clawdia.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `clawdia status --all` (pasteable, secrets redacted) over raw logs.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.clawdia/sandboxes`
