---
summary: "Quick troubleshooting guide for common Clawdia failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When Clawdia misbehaves, here's how to fix it.
...
|---|---|---|
| `clawdia status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `clawdia status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `clawdia status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `clawdia gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `clawdia channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `clawdia gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `clawdia logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `clawdia status --all` (it redacts tokens). If you paste `clawdia status`, consider setting `CLAWDIA_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  clawdia models auth setup-token --provider anthropic
  ```
...
log in with Claude Code CLI on the gateway host, then run `clawdia models status`
to sync the refreshed token into Clawdia’s auth store.
...
**Logs:**
- Preferred: `clawdia logs --follow`
- File logs (always): `/tmp/nelsonmuntz-c/clawdia-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$CLAWDIA_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u clawdia-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  clawdia config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  clawdia config set gateway.mode remote
  clawdia config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `clawdia setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.clawdia/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `clawdia doctor`
...
- then run `clawdia sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `clawdia gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `clawdia gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `clawdia gateway status` and `clawdia doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `CLAWDIA_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `clawdia gateway status` shows a config mismatch**
...
- Fix: rerun `clawdia gateway install --force` from the same `--profile` / `CLAWDIA_STATE_DIR` you want the service to use.

**If `clawdia gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `clawdia doctor` to update it (or `clawdia gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `CLAWDIA_GATEWAY_TOKEN`) and restart the service.

**If `clawdia gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/clawdia` on disk.
...
Symptoms: `pwd` or file tools show `~/.clawdia/sandboxes/...` even though you
...
Clawdia intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `clawdia models list` or
  `clawdia models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${CLAWDIA_CONFIG_PATH:-$HOME/.nelsonmuntz-c/clawdia.json}"
```
...
tail -f "$(ls -t /tmp/nelsonmuntz-c/clawdia-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
clawdia logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.clawdia/agents/<agentId>/sessions/
```
...
clawdia logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
clawdia gateway --verbose
```

If you’re logged out / unlinked:

```bash
clawdia channels logout
trash "${CLAWDIA_STATE_DIR:-$HOME/.clawdia}/credentials" # if logout can't cleanly remove everything
clawdia channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/nelsonmuntz-c/clawdia-*.log | head -1)" | tail -20
```

### High Memory Usage

Clawdia keeps conversation history in memory.
...
Clawdia now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `clawdia doctor` reports every invalid entry.
- `clawdia doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `clawdia logs`, `clawdia health`, `clawdia status`, `clawdia gateway status`, and `clawdia gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/clawdia/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `clawdia models status` (CLI).
...
5) Run `clawdia channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Clawdia scrubs/normalizes tool
...
```bash
tccutil reset All com.clawdia.mac.debug
```

**Fix 2: Force New Bundle ID**
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/nelsonmuntz-c/clawdia/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/com.clawdia.gateway (replace with com.clawdia.<profile> if needed)
```
...
Ensure the global `clawdia` CLI is installed and matches the app version:
```bash
clawdia --version
...
#   ${CLAWDIA_CONFIG_PATH:-$HOME/.nelsonmuntz-c/clawdia.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
clawdia gateway --verbose
clawdia channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/nelsonmuntz-c/clawdia-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$CLAWDIA_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.clawdia/logs/...`; profiles use `~/.clawdia-<profile>/logs/...`)<br />Linux: `journalctl --user -u clawdia-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Clawdia Gateway (<profile>)" /V /FO LIST` |
| Session files | `$CLAWDIA_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$CLAWDIA_STATE_DIR/media/` |
| Credentials | `$CLAWDIA_STATE_DIR/credentials/` |
...
clawdia gateway status --deep

# Is the gateway reachable?
clawdia health --json
# If it fails, rerun with connection details:
clawdia health --verbose
...
tail -20 /tmp/nelsonmuntz-c/clawdia-*.log
```
...
trash "${CLAWDIA_STATE_DIR:-$HOME/.clawdia}"
clawdia channels login         # re-pair WhatsApp
clawdia gateway restart           # or: clawdia gateway
```
...
1. Check logs first: `/tmp/clawdia/` (default: `clawdia-YYYY-MM-DD.log`, or your configured `logging.file`)
