...
3) Set tokens for Clawdia and start the gateway.
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, Clawdia falls back to the user token.
...
```json
{
  "display_information": {
    "name": "Clawdia",
    "description": "Slack connector for Clawdia"
  },
  "features": {
    "bot_user": {
    ...
    "slash_commands": [
      {
        "command": "/clawd",
        "description": "Send a message to Clawdia",
        "should_escape": false
      }
    ]
  },
...
- `allowlist` requires channels to be listed in `channels.slack.channels`.
 ...
 - On startup, Clawdia resolves channel/user names in allowlists to IDs (when tokens allow)
