...
read_when:
  - You want Clawdia to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Clawdia to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`clawdia onboard`) and `clawdia channels add` list optional channel plugins.
...
```bash
clawdia plugins install @clawdia/nostr
```

Use a local checkout (dev workflows):

```bash
clawdia plugins install --link <path-to-clawdia>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "clawdia@example.com",
        "lud16": "clawdia@example.com"
      }
    }
  }
}
