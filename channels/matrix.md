...
Matrix is an open, decentralized messaging protocol. Clawdia connects as a Matrix **user**
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Clawdia calls the same
     login endpoint, stores the access token in `~/.clawdia/credentials/matrix/credentials.json`,
...
`~/.clawdia/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
