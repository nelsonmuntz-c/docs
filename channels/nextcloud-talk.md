...
```bash
clawdia plugins install @clawdia/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
clawdia plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Clawdia" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `clawdia pairing list nextcloud-talk`
  - `clawdia pairing approve nextcloud-talk <CODE>`
