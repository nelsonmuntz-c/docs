...
Clawdia can convert outbound replies into audio using ElevenLabs, OpenAI, or Edge TTS.
It works anywhere Clawdia can send audio; Telegram gets a round voice-note bubble.
...
Edge TTS does **not** require an API key. If no API keys are found, Clawdia defaults
...
TTS config lives under `messages.tts` in `clawdia.json`.
...
- If `provider` is **unset**, Clawdia prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `edge`.
...
`~/.clawdia/settings/tts.json`, override with `CLAWDIA_TTS_PREFS` or
...
Discord note: `/tts` is a built-in Discord command, so Clawdia registers
