---
summary: "First-run onboarding flow for Clawdia (macOS app)"
read_when:
...
- Writes credentials to `~/.clawdia/credentials/oauth.json`
...
The app can install the global `clawdia` CLI via npm/pnpm so terminal
...
On the first agent run, Clawdia bootstraps a workspace (default `~/clawd`):
...
```bash
clawdia webhooks gmail setup --account you@gmail.com
```
...
- `~/.clawdia/credentials/oauth.json`
