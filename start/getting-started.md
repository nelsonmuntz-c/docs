...
Recommended path: use the **CLI onboarding wizard** (`clawdia onboard`). It sets up:
...
- **Recommended:** Brave Search API key for web search. Easiest path:
  `clawdia configure --section web` (stores `tools.web.search.apiKey`).
...
If `clawdia health` shows “no auth configured”, go back to the wizard and set OAuth/key auth — the agent won’t be able to respond without it.

Tip: `clawdia status --all` is the best pasteable, read-only debug report.
Health probes: `clawdia health` (or `clawdia status --deep`) asks the running gateway for a health snapshot.
