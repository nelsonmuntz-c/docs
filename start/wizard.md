...
The onboarding wizard is the **recommended** way to set up Clawdia on macOS,
...
(`web_fetch` works without a key). Easiest path: `clawdia configure --section web`
...
1) **Existing config detection**
   - If `~/.nelsonmuntz-c/clawdia.json` exists, choose **Keep / Modify / Reset**.
   - Re-running the wizard does **not** wipe anything unless you explicitly choose **Reset**
     (or pass `--reset`).
   - If the config is invalid or contains legacy keys, the wizard stops and asks
     you to run `clawdia doctor` before continuing.
...
2) **Model/Auth**
   ...
   - **OpenAI API key**: uses `OPENAI_API_KEY` if present or prompts for a key, then saves it to `~/.clawdia/.env` so launchd can read it.
  ...
  - OAuth credentials live in `~/.clawdia/credentials/oauth.json`; auth profiles live in `~/.clawdia/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
...
5) **Channels**
  ...
  - DM security: default is pairing. First DM sends a code; approve via `clawdia pairing approve <channel> <code>` or use allowlists.
...
7) **Health check**
   - Starts the Gateway (if needed) and runs `clawdia health`.
   - Tip: `clawdia status --deep` adds gateway health probes to status output (requires a reachable gateway).
...
Use `clawdia agents add <name>` to create a separate agent with its own workspace,
...
- Stores it under `~/.clawdia/tools/signal-cli/<version>/`.
...
Typical fields in `~/.nelsonmuntz-c/clawdia.json`:
...
`clawdia agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.clawdia/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.clawdia/agents/<agentId>/sessions/`.
