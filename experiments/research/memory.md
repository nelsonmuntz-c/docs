...
read_when:
  ...
  - Deciding: standalone CLI vs deep Clawdia integration
...
```
## Retain
- W @User: Currently in vacation.
- B @workspace: I fixed the Baileys WS crash by wrapping connection.update handlers in try/catch (see memory/2025-11-27.md).
- O(c=0.95) @User: Prefers concise replies (&lt;1500 chars) on WhatsApp; long content goes into files.
```

Minimal parsing:
- Type prefix: `W` (world), `B` (experience/biographical), `O` (opinion), `S` (observation/summary; usually generated)
- Entities: `@User`, `@workspace`, etc (slugs map to `bank/entities/*.md`)
...
Recommendation: **deep integration in Clawdia**, but keep a separable core library.

### Why integrate into Clawdia?
- Clawdia already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `clawdia memory recall "…" --k 25 --since 30d`
  - `clawdia memory reflect --since 7d`
