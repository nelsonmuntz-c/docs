...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Clawdia
---

# Node + tsx "__name is not a function" crash

## Summary
Running Clawdia via Node with `tsx` fails at startup with:

```
[clawdia] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `clawdia status` and `gateway:watch` worked.
