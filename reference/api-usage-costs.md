...
Clawdia features that can generate provider usage or paid API calls.
...
- `clawdia status --usage` and `clawdia channels list` show provider **usage windows**
...
Clawdia can pick up credentials from:
...
- `clawdia status --usage`
- `clawdia models status --json`
...
### 8) Model scan / probe
`clawdia models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
