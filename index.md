---
summary: "Top-level overview of Clawdia, features, and purpose"
read_when:
  - Introducing Clawdia to newcomers
---
# Clawdia 🦞
...
- **Guided setup (recommended):** [Wizard](/start/wizard) (`clawdia onboard`)
- **Open the dashboard (local Gateway):** http://127.0.0.1:18789/ (or http://localhost:18789/)

If the Gateway is running on the same computer, that link opens the browser Control UI
immediately. If it fails, start the Gateway first: `clawdia gateway`.
...
WhatsApp / Telegram / Discord / iMessage (+ plugins)
  ...
  │                           │    /__clawdia__/canvas/ (Canvas host)
  └───────────┬───────────────┘
              │
              ├─ Pi agent (RPC)
              ├─ CLI (clawdia …)
              ├─ Chat UI (SwiftUI)
              ├─ macOS app (Clawdia.app)
              ├─ iOS node via Gateway WS + pairing
              └─ Android node via Gateway WS + pairing
```

Most operations flow through the **Gateway** (`clawdia gateway`), a single long-running process that owns channel connections and the WebSocket control plane.
...
- **Loopback-first**: Gateway WS defaults to `ws://127.0.0.1:18789`.
  ...
  - For Tailnet access, run `clawdia gateway --bind tailnet --token ...` (token is required for non-loopback binds).
- **Nodes**: connect to the Gateway WebSocket (LAN/tailnet/SSH as needed); legacy TCP bridge is deprecated/removed.
- **Canvas host**: HTTP file server on `canvasHost.port` (default `18793`), serving `/__clawdia__/canvas/` for node WebViews; see [Gateway configuration](/gateway/configuration) (`canvasHost`).
...
Switching between npm and git installs later is easy: install the other flavor and run `clawdia doctor` to update the gateway service entrypoint.
...
If you don’t have a global install yet, run the onboarding step via `pnpm clawdia ...` from the repo.
...
- Start here:
  ...
  - [Clawdia assistant setup (Clawd)](/start/clawd)
  - [Skills](/tools/skills)
