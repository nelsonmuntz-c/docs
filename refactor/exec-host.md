...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.clawdia/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.clawdia/node.json` (node id + pairing token).
...
Path: `~/.clawdia/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.clawdia/exec-approvals.sock",
...
### IPC
- Unix socket at `~/.clawdia/exec-approvals.sock` (0600).
