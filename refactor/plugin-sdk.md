...
Delivery:
- Publish as `@clawdia/plugin-sdk` (or export from core under `clawdia/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `ClawdiaPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ClawdiaConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ClawdiaConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ClawdiaConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: ClawdiaConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ClawdiaConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ClawdiaConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@clawdia/plugin-sdk`.
- Add `api.runtime` to `ClawdiaPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `clawdiaRuntime: ">=2026.2.0"`).
