---
summary: "Install Clawdia (recommended installer, global install, or from source)"
read_when:
  - Installing Clawdia
...
```bash
curl -fsSL https://clawdia.cc/install.sh | bash -s -- --no-onboard
```
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g clawdia@latest
```
...
```bash
# Explicit npm
curl -fsSL https://clawdia.cc/install.sh | bash -s -- --install-method npm
...
If `$(npm prefix -g)/bin` (macOS/Linux) or `$(npm prefix -g)` (Windows) is **not** present inside `echo "$PATH"`, your shell can’t find global npm binaries (including `clawdia`).
