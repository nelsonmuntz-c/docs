...
read_when:
  - You want to understand `clawdia.cc/install.sh`
...
If the installer completes but `clawdia` is not found in a new terminal, it’s usually a Node/npm PATH issue. See: [Install](/install#nodejs--npm-path-sanity).
...
This script installs `clawdia` into a prefix (default: `~/.clawdia`) and also installs a dedicated Node runtime under that prefix, so it can work on machines where you don’t want to touch the system Node/npm.
