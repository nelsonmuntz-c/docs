...
read_when:
  - You installed Clawdia but `clawdia` is “command not found”
...
Clawdia’s runtime baseline is **Node 22+**.

If you can run `npm install -g clawdia@latest` but later see `clawdia: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
