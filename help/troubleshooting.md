...
You can also set `CLAWDIA_VERBOSE=1` instead of the flag.
...
### `docs.clawdia.cc` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.clawdia.cc` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.clawdia.cc` to the allowlist, then retry.
...
- Check the allowlist: `clawdia config get agents.defaults.models`
...
If you can, include the relevant log tail from `clawdia logs --follow`.
